		...
		dojo.declare('game', null, {
			...
			constructor: function(args){
				...
				self.replay = false;
				uow.ui.connectKeys();
				dojo.subscribe('/uow/key/up', self, 'processKeyUp');
				dojo.subscribe('/uow/key/down', self, 'processKeyDown');
				self.setup();
			},
			setup: function(){
				var self = this;
			...
			setupAudio: function(){
				...
				self.say = function(words, channel){
					...
					self.noise.setProperty({name : 'rate', value : this.speechRate, channel : channel, immediate : true});
					self.noise.setProperty({name : 'volume', value : this.masterVolume*this.speechVolume, channel : channel, immediate : true});
					self.noise.say({channel:channel,text:words});
				}
				self.play = function(url, channel){
					self.noise.stop({channel:channel});
					self.noise.setProperty({name : 'volume', value : this.masterVolume*this.soundVolume, channel : channel, immediate : true});
				...
				self.changeVol = function(channel, d){
					...
					self.noise.setProperty({channel:channel, name:'volume', value: newvol*this.masterVolume*this.soundVolume, immediate:true});
				}
			},
			setupGraphics: function(){
				var self = this;
				self.r = new Raphael(0,0,self.dim.w,self.dim.h);
				console.log('setting up the background image in setupgraphics');
				self.back = self.r.image('tunnback.png',0,0,self.dim.w,self.dim.h).toBack();
			},
			...
			intro: function(){
				var self = this;
				console.log('inside of intro, where the text stuff is supposed to be set up', self.dim.cx, self.dim.cy);
				...
				console.log('here is how it turned out', self.introtext, self.introback);
			},
			changeMode: function(){
				var self = this;
			...
			finish: function(outcome){
				...
				if (outcome == 'lose'){
					var message = 'You were killed by the wily tunnel!  Your score is '+self.score*134;
				}
				var b = self.r.text(self.dim.cx, self.dim.cy, message+'\nPress down to start again!').attr({font:'30px Arial', fill:'#fff'}).toFront();
				self.r.rect(-30+b.getBBox().x, -30+b.getBBox().y, 60+b.getBBox().width, 60+b.getBBox().height).attr({fill:'#000'});
				b.toFront();
				self.okaytorestart = false;
				self.noise.stop({channel:'main'});
				self.noise.say({text: message + ' Press down to play again!', channel:'main'}).callAfter(function(){
